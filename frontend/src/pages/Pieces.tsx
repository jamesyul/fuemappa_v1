...
const Pieces: React.FC = () => {
  ...
  const { pieces, fetchPieces, setFilter, setDepartmentFilter, filteredPieces } = usePiecesStore();
  ...
  useEffect(() => {
    ...
    if (user.role === 'jefe_departamento' || user.role === 'integrante_departamento') {
      if (user.departmentId) { setDepartmentFilter(user.departmentId); } 
      else { setDepartmentFilter('d1'); } // Fallback
    } else {
      setDepartmentFilter('');
      setFilter('');
    }
  ...
  const handleCreatePiece = async (piece: Piece, imageFile?: File | null) => {
    try {
      const pieceDataForApi = {
        code: piece.code,
        name: piece.name,
        departmentId: piece.departmentId,
  ...
  const handleEditPiece = async (piece: Piece, imageFile?: File | null) => {
    ...
    try {
      const pieceDataForApi = {
        code: piece.code, name: piece.name, departmentId: piece.departmentId,
        quantity: Number(piece.quantity), price: Number(piece.price), report: piece.report,
      };
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6">
      ...
      <div className="mb-4 flex space-x-4">
        ...
        />
        <button onClick={() => alert('Filtro avanzado no implementado aún')} className="bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300">
          Filtro Avanzado
        </button>
      </div>
              ...
              filteredPieces.map((piece: Piece) => (
                <tr key={piece.id} className="border-t border-gray-200 hover:bg-gray-50">
                  ...
                  <td className="py-3 px-4 text-sm text-gray-700">
                    <button onClick={() => { setSelectedPiece(piece); setIsReportOpen(true); }} className="text-indigo-600 hover:text-indigo-800">Ver informe</button>
                  </td>
                  {canEditDelete && (
